...
function FoodItem() {
  ...
  if (!pet) return <p className="text-[0.75rem] xs:text-[1rem] sm:text-[1.25rem] text-center px-[1.5rem]">Loading pet data...</p>

  const inventoryItem = pet.inventory.find((i) => i.itemName === itemId)
  if (!inventoryItem) {
    return (
      <div className="flex flex-col justify-center items-center gap-8">
        <p className="text-center px-[2rem] text-[0.75rem] xs:text-[1rem] sm:text-[1.25rem]">Item not currently found in your inventory.</p>
  ...
  if (!storeItem) return <p className="text-[0.75rem] xs:text-[1rem] sm:text-[1.25rem] text-center px-[1.5rem]">Loading item details...</p>
