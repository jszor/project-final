...
function Medicine() {
  ...
  if (loading) return <p className="text-center px-[1.5rem] text-[0.75rem] xs:text-[1rem] sm:text-[1.5rem]">Loading medicine...</p>
  if (error) return <p className="text-center text-[0.75rem] xs:text-[1rem] sm:text-[1.5rem]">Error: {error}</p>
  
  if (!medicine.length) {
    return (
      <div className="flex flex-col justify-center items-center gap-8">
        <p className="text-center px-[3rem] text-[0.75rem] xs:text-[1rem] sm:text-[1.5rem]">Currently no medicine in inventory.</p>
  ...
  return (
    <>
      <h2 className="text-[0.75rem] xs:text-[1rem] sm:text-[1.5rem]">MEDICINE:</h2>
