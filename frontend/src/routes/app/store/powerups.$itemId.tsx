...
export const Route = createFileRoute('/app/store/powerups/$itemId')({
  component: PowerupItem,
})

function PowerupItem() {
  const { itemId } = useParams({ from: '/app/store/powerups/$itemId' })
  ...
  if (!storeItems.length) return <p className="text-center px-[1.5rem] text-[0.75rem] xs:text-[1rem] sm:text-[1.25rem]">Loading store items...</p>
  ...
  if (!storeItem) {
    return (
      <div className="flex flex-col justify-center items-center gap-8">
        <p className="text-center px-[2rem] text-[0.75rem] xs:text-[1rem] sm:text-[1.25rem]">Item not found in the store.</p>
        <Link to="/app/store/powerups">
  ...
  return (
    <div className="flex flex-col h-full justify-center items-center text-ammo-100 text-[0.75rem] xs:text-[1rem] sm:text-[1.25rem] gap-[2rem]">
      ...
      {message && <p className="text-ammo-100 text-center px-[2rem] text-[0.75rem] xs:text-[1rem] sm:text-[1.25rem]">"{message}"</p>}
